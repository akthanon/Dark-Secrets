...
draw_set_font(font0);
if global.dialogm=false and !collision_rectangle( x+17, y+32, x-17, y-32, NPC_Padre, false, true ) and !place_meeting(personaje.x,personaje.y,checkpoint) and !place_meeting(personaje.x,personaje.y,objeto_padre) and !place_meeting(personaje.x,personaje.y,escalera) and (!mouse_check_button(mb_left) or finalizado=false)
{
if (keyboard_check_pressed(ord('E')) or mouse_check_button_pressed(mb_left)) and !(keyboard_check(ord('E')) and mouse_check_button(mb_left)) and abs(personaje.x-x)<16 and place_meeting(x,y,personaje) and presz=true
{
...
else if keyboard_check_pressed(ord('E')) and !audio_is_playing(f_error) and abs(personaje.x-x)<16 and place_meeting(x,y,personaje) and !mouse_check_button(mb_left) and global.dialogm=false {audio_play_sound(f_error,0,0)}
